...
export class CLI {
    ...
    parseArguments() {
        ...
        if (values["max-width"]) {
            ...
            if (isNaN(parsed) || parsed < 1) {
                console.error(`Invalid max-width: ${values["max-width"]}. Must be a positive integer.`);
                process.exit(1);
            }
            maxWidth = parsed;
        }
    ...
    async run() {
        const { positionals, theme, focusLine, contextLines, showLineNumbers, wrapLines, truncateLines, maxWidth, diagnostics, splitDiagnostics } = this.parseArguments();
